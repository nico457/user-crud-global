...
|--------|---------------------------|------------------------------------------|------------------------------------------------|-----------------------------|
| GET    | `/api/v1/users`           | Obtiene la lista de todos los usuarios.  |<b>Query Params opcionales</b> - `page` Número de página<br>- `limit` Cantidad de usuarios por página <br>-`sortBy` Campo para ordenamiento -`order` asc o desc<br>-`q` Texto libre para buscar por username, email, nombre o apellido. <br>- `role` Filtra usuarios por rol. (ADMIN,USER,SUBSCRIBER) | Lista de usuarios. |
